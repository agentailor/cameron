...
### 1. Agent Factory (`src/lib/agent/index.ts`)

The heart of the AI agent system. It builds the agent with LangChain v1's `createAgent` — a
prebuilt ReAct agent that runs on LangGraph (`model` node → `tools` node loop) — and installs
`humanInTheLoopMiddleware` to gate mutating tools for human approval.

```typescript
import { createAgent, humanInTheLoopMiddleware } from "langchain";

// Only mutating tools pause for approval; reads and MCP tools auto-approve.
const MUTATING_TOOL_NAMES = ["log_expense", "import_transactions_csv"] as const;

async function buildAgent(cfg?: AgentConfigOptions) {
  const llm = createChatModel({ provider, model: modelName, temperature: 1 });
  const allTools = [...builtinTools, ...configTools, ...mcpTools];
...
- Agent construction via `createAgent` (prebuilt ReAct agent over LangGraph)
- Per-tool human-in-the-loop approval via `humanInTheLoopMiddleware` (mutations only)
- Model configuration (`createChatModel`) and system prompt
...
export async function streamResponse(params: {
  ...
  // from the checkpoint to emit one decision per proposed action (approve / reject).
  let inputs;
  if (opts?.allowTool) {
    const decision =
    ...
    const pending = snapshot.tasks?.flatMap((t) => t.interrupts ?? []).map((i) => i.value)[0];
    const count = pending?.actionRequests?.length ?? 1;
    inputs = new Command({ resume: { decisions: Array.from({ length: count }, () => decision) } });
  } else {
    inputs = { messages: [new HumanMessage(userText)] };
  }
...
`createAgent` compiles the standard ReAct loop: a `model` node calls the LLM (with the system
prompt and bound tools), and a `tools` node executes any tool calls, looping back to `model` until
there are no more tool calls. `humanInTheLoopMiddleware` runs after the `model` node and, for a
**mutating** tool call, raises an interrupt (pause) before the `tools` node runs.
...
```
    START
      ...
      ▼
┌──────────┐
│  model   │ ──► Call language model with tools (system prompt injected)
└──────────┘
      │
      ▼
  humanInTheLoop
  (mutating tool?)
   ┌─────────┐
   │   Yes   │ ──► interrupt ──► Human review (approve / reject)
   └─────────┘                        │
   ┌─────────┐                        ▼
   │   No    │ ───────────────► ┌─────────┐
   └─────────┘                  │  tools  │ ──► Execute tools
      │                         └─────────┘
      ▼                              │
     END  ◄── no more tool calls ────┘  (else back to model)
```
...
- **Process**: `createAgent` injects the system prompt, binds tools, and generates a response
- **Output**: AI message (text and/or tool calls)

#### Human-in-the-Loop Middleware (afterModel)

- **Input**: AI message with tool calls
- **Process**:
  - For tools listed in `interruptOn` (mutations only), batch them into one `HITLRequest` and interrupt
  - Non-listed tools (reads, MCP) auto-approve and proceed
  - When resumed, apply each decision: **approve** (run as-is) or **reject** (inject a tool result and return to the model)
- **Output**: Proceeds to the `tools` node, or back to `model` on reject

#### Tools Node

- **Input**: Approved tool calls
- **Process**: Execute built-in and MCP tools
- **Output**: Tool results as messages

### Interrupt Handling

The middleware surfaces a batched `HITLRequest` and is resumed with a matching `HITLResponse` — one
decision per proposed action. The service (`agentService.ts`) reads the pending request from the
checkpoint and builds the decisions from the client's `allow` / `deny` signal:

```typescript
import { Command } from "@langchain/langgraph";
import type { Decision, HITLResponse } from "langchain";
...
// One decision per proposed action, positionally aligned to the request.
const snapshot = await agent.graph.getState({ configurable: { thread_id: threadId } });
const pending = snapshot.tasks?.flatMap((t) => t.interrupts ?? []).map((i) => i.value)[0];
const count = pending?.actionRequests?.length ?? 1;

const resume: HITLResponse = { decisions: Array.from({ length: count }, () => decision) };
await agent.stream(new Command({ resume }), { configurable: { thread_id: threadId } });
```
...
Only **mutating** tools (`log_expense`, `import_transactions_csv`) trigger this flow; read-only and
MCP tools run without pausing.

### Approval Options

#### 1. Allow

- **Action**: Execute the tool with the original parameters
- **Implementation**: resume with `{ decisions: [{ type: "approve" }] }`
- **Result**: Tool runs and the agent continues with the result

#### 2. Deny

- **Action**: Skip tool execution
- **Implementation**: resume with `{ decisions: [{ type: "reject", message }] }`
- **Result**: The middleware injects a tool result and the model produces a follow-up message explaining nothing was changed
