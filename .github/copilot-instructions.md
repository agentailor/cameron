...
- **LangChain Agent**: Built with LangChain v1's `createAgent` (prebuilt ReAct agent over LangGraph) in `src/lib/agent/index.ts` — assembles model, tools, and middleware
...
- **Tool Approval**: `humanInTheLoopMiddleware` gates **mutating** tools only (`interruptOn` map); read/MCP tools auto-approve, `approveAllTools` skips the gate
...
```bash
# Setup (requires Postgres running on port 5544)
docker compose up -d
pnpm install
pnpm db:migrate
...
pnpm db:studio  # Database UI (Drizzle Studio)
...
- `humanInTheLoopMiddleware` pauses on a mutating tool call and emits a batched HITL interrupt (read/MCP tools don't pause)
- Frontend detects the pending call from `tool_calls` on the last AI message, shows the approval UI, and re-opens the stream with `allowTool=allow/deny`
- `agentService.ts` reads the pending request via `agent.graph.getState()` and resumes with `Command({ resume: { decisions } })` (`allow`→approve, `deny`→reject)
