...
| **v2**  | Memory + evals          | A regression eval suite over the existing capabilities; budgets and preferences as long-term memory.             |
| **v3**  | External data           | A standalone bank MCP server Cameron consumes as a client.                                                       |
| **v4+** | Skills → self-extension | Skills, document ingestion, autonomy, channels, delegation, and eventually Cameron writing its own capabilities. |
